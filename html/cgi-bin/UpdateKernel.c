...
#define  FILE_1M 	1024*1024*1
#define  FILE_2M    1024*1024*2	
#define  BufferLen  1024
#define  DATAFILE  "/tmp/uImage" 
...
unsigned int  do_pc_crc(char *file_name) 
{
	...
	char number[4];
	/*VPC6467TDRV4.1_5678.gz*/
	if ((p =strrchr(file_name, '_')) != NULL)
	{
		strncpy(number,p+1,4);
		crc_pc = strtol(number,NULL,16);
	}
	else
	{
...
{ 
	...
	if(cgiFormFileName("K_Update", name, sizeof(name)) != cgiFormSuccess)
	{
		printf("{msg:'文件名字为空'\n}");
		return -1;
	}
	
	cgiFormFileSize("K_Update",&filesize);
	
	if((filesize < FILE_1M)||(filesize > FILE_2M))
	{
		printf("{msg:'请选择大小正确的文件'\n}");
		return -1;
	}
	
	if (cgiFormFileOpen("K_Update", &srcfile) != cgiFormSuccess) 
	{
	...
	if(crc_local == crc_pc)
	{
		system("/usr/sbin/flash_eraseall /dev/mtd1");
		system("/usr/sbin/nandwrite -p -q /dev/mtd1 /tmp/uImage");
		//system("/usr/bin/killall Mp2ToH264");
		system("/bin/rm /tmp/*");
		printf("{msg:'升级内核成功,请手动重启设备'\n}\n");
		return 0;
	}
	else
	{
